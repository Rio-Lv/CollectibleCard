...
function BaseCard10(props) {
  const backRef = useRef();
  const cardRef = useRef();
  const [shrink, set_shrink] = useState(props.shrink);
  const [title_mode, set_title_mode] = useState(props.titleMode);
  //hover effects
  ...
  useEffect(() => {
    ...
    // console.log(props.k);
    set_title_mode(props.titleMode);
  }, [props.k, props.shrink, props.url, props.titleMode]);
  ...
  useEffect(() => {
    ...
    set_card_width(k);
    set_card_height(title_mode ? k / 15 : k * 1.35 - (k - props.shrink * k));
    ...
    set_title_size(title_mode ? k / 20 : k / 22);
    set_cycle_size(k / 6);
    set_name_bot(-k / 90);
    set_cycle_bot(k / 13.5);
    set_image_top(k * 0);
  }, [k, props.shrink, title_mode]);
  ...
  }, [image_width, imagebox_height, props.shrink,title_mode]);
  ...
  }, [image_height, traptop_top, props.shrink,title_mode]);

  // sixt order
  useEffect(() => {
    set_title_top(title_mode?traptop_top_inner + k / 23.5:traptop_top_inner + k / 34);
  }, [traptop_top_inner, props.shrink, title_mode]);
  ...
  return (
    <Base
      ...
      onClick={(e) => {
        console.log(
          `X:${percentX.toFixed(3) * 100}, Y:${percentY.toFixed(3) * 100}`
        );
      }}
    >
      <div
        ...
        >
          ...
          <Cycle
            style={{
              ...
              color: `${title_mode ? "tranparent" : cycle_color}`,
              backgroundClip: "text",
            }}
          >
            18723
          ...
          <Name
            style={{
              bottom: `${name_bot}vw`,
              fontSize: `${title_size}vw`,
              color: `${title_mode ? "transparent" : name_color}`,
            }}
          >
